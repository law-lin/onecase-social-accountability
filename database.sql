...
drop type if exists public.user_relationship_status cascade;
...
create type public.user_relationship_status as enum ('pending_one_two', 'pending_two_one', 'friends', 'block_one_two', 'block_two_one', 'block_both');
create type public.notification_type as enum ('assign_task', 'add_council', 'accept_council', 'add_friend', 'accept_friend', 'nudge', 'comment', 'comment_reply');
-- USERS
create table public.users (
  ...
  push_token          text,
  has_failed_clock_in boolean default false,
...
create table public.user_relationships (
  user_one_id       uuid references public.users on delete cascade not null,
  user_two_id       uuid references public.users on delete cascade not null,
  status            user_relationship_status not null,
  time_sent         timestamp with time zone default timezone('utc'::text, now()),
...
create table public.tasks (
  ...
  created_by    uuid references public.users not null,
  assigned_to   uuid references public.users on delete cascade,
...
create table public.clock_ins (
  ...
  task_id       bigint references public.tasks not null
);
...
comment on table public.task_comments is 'Comment sent by a user for a task.';
...
create table public.notifications (
  ...
  case_id         bigint references public.cases on delete cascade,
  task_id         bigint references public.tasks on delete cascade,
  task_comment_id bigint references public.task_comments on delete cascade
);
...
-- create policy "Allow individual update access" on public.tasks for update using ( auth.uid() = assigned_to or auth.uid() = created_by);
-- create policy "Allow individual delete access" on public.tasks for delete using ( auth.uid() = assigned_to or auth.uid() = created_by);
