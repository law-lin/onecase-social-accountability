...
import useUpdateCase from '../mutations/useUpdateCase';
import { useMixpanel } from '../providers/MixpanelContext';

interface Props {
  navigation: StackNavigationProp<StackParamList, 'UpdateCase'>;
  route: RouteProp<StackParamList, 'UpdateCase'>;
}

function UpdateCaseScreen({ navigation, route }: Props) {
  const { caseItem } = route.params;

  const [title, setTitle] = useState(caseItem.title);
  const [search, setSearch] = useState('');
  const [users, setUsers] = useState<User[]>(
    caseItem.usersCases?.map((usersCase) => ({
      ...usersCase.users,
      selected: true,
    })) as User[]
  );
  const [emoji, setEmoji] = useState(caseItem.emoji);
  ...
  useEffect(() => {
    if (isSuccess && friends) {
      ...
      const unselectedFriends = friends.filter(
        (friend) => !existingUserIds.includes(friend.id)
      );
      // console.log('USRSE', users);
      // console.log('ALL IDS', existingUserIds);
      // console.log('FRINEDLIST', unselectedFriends);
      const friendList = unselectedFriends?.map((friend: User) => {
        friend.selected = false;
        return friend;
      });
      setUsers((oldUsers) => oldUsers.concat(friendList));
    }
  }, [isSuccess]);
  ...
  const updateTask = async () => {
    ...
    await updateCaseMutation.mutateAsync(
      { caseId: caseItem.id, title, emoji, users: ids },
      {
        onSuccess: async (data) => {
          mixpanel?.track('Updated case', {
            'Current Number of Council Members': ids.length,
            'Number of Council Members Added': data.numAdded,
            'Number of Council Members Removed': data.numRemoved,
          });
          queryClient.invalidateQueries('cases');
          // if (data && data.length > 0) {
          //   console.log('CASE DATA ASSIGN', data);
        ...
        <View style={{ flex: 1 }}>
          ...
          >
            <Text style={styles.title}>
              Update <Text style={styles.case}>Case</Text> Title
            </Text>
            <View
